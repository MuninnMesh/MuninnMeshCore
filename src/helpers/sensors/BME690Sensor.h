...
private:
  ...
  uint8_t calcResHeat(uint16_t temp_c) {
    // Official BME690 heater DAC transfer (Bosch BME690_SensorAPI bme69x.c
    // calc_res_heat, integer version). NOTE: this is NOT the BME680/688
    // formula — the BME690 uses new constants (par_g1+784, par_g2+154009,
    // x34/100). Using the 680 formula programs a garbage setpoint: the gas
    // plate rails at ADC max and reads back as a constant bogus resistance
    // (the infamous frozen 5684 ohms) with valid/stable bits set.
