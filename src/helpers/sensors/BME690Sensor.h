...
private:
  ...
  uint8_t calcResHeat(uint16_t temp_c) {
    float target = (float)(temp_c > 400 ? 400 : temp_c);
    float var1 = ((float)_c.g1 / 16.0f) + 49.0f;
    float var2 = (((float)_c.g2 / 32768.0f) * 0.0005f) + 0.00235f;
    float var3 = (float)_c.g3 / 1024.0f;
    float var4 = var1 * (1.0f + var2 * target);
    float var5 = var4 + var3 * _amb_temp;
    float term_range = 4.0f / (4.0f + (float)_c.res_heat_range);
    float term_val = 1.0f / (1.0f + ((float)_c.res_heat_val * 0.002f));
    float hv = 3.4f * ((var5 * term_range * term_val) - 25.0f);
    if (hv < 0.0f) hv = 0.0f;
    if (hv > 255.0f) hv = 255.0f;
    return (uint8_t)hv;
  }
