...
void WiFiOTA::scan(char* reply) {
  ...
  for (int i = 0; i < m; i++) {
    ...
    char entry[24];
    int len = snprintf(entry, sizeof(entry), "%s%s:%d", (shown ? " | " : ""), name, (int)WiFi.RSSI(order[i]));
  ...
  else if (shown < n) snprintf(&reply[pos], REPLY_CAP - pos, " (+%d more)", n - shown);
...
void WiFiOTA::status(char* reply) {
  switch (phase) {
    case OTA_IDLE:      strcpy(reply, scan_pending ? "idle (scan running)" : "idle"); break;
...
void WiFiOTA::loop() {
  ...
  if (phase == OTA_HTTP_STARTING) {
    ...
    if (memcmp(ota_url, "https://", 8) == 0) {
      ...
      tls->setInsecure();   // per project decision: transport auth not required
      client = tls;
